...
<!--
   ...
   PixelPin Social Login Plugin ( <?php echo $_SESSION["wsl::plugin"] ?> ) 
   http://wordpress.org/extend/plugins/pixelpin-social-login/
-->
<?php 
...
{ 
	...
	$assets_base_url  = WORDPRESS_SOCIAL_LOGIN_PLUGIN_URL . '/assets/img/32x32/' . $social_icon_set; 
?> 
<h3><?php _wsl_e("Social networks", PIXELPIN_PLUGIN_CATEGORY) ?></h3> 
<table class="form-table">  
	<tr>  
		<td valign="top">
			<table id="wsl-user-profile-injected-table-b">
				<tr>
					<th width="80"><?php _wsl_e("Provider", PIXELPIN_PLUGIN_CATEGORY) ?></th>
					<th><?php _wsl_e("Identity", PIXELPIN_PLUGIN_CATEGORY) ?></th> 
				</tr>
				<?php
...
?>	
	<tr>    
		<td valign="top">
			<b><?php _wsl_e("Add more identities", PIXELPIN_PLUGIN_CATEGORY) ?></b>
			<br />
			<?php
				foreach( $WORDPRESS_SOCIAL_LOGIN_PROVIDERS_CONFIG AS $item ){
					$provider_id   = @ $item["provider_id"];
...
?>
	<tr> 
		<td>
			<b><?php _wsl_e("Currently connected to:", PIXELPIN_PLUGIN_CATEGORY) ?></b> 
