...
function wsl_process_login_checks()
{
	...
	if(  $_REQUEST[ 'action' ] == "wordpress_social_link" && ! is_user_logged_in() ){
		wsl_render_notices_pages( _wsl__("Bouncer say don't be silly!", PIXELPIN_PLUGIN_CATEGORY) );

		return false;
	}

	if(  $_REQUEST[ 'action' ] == "wordpress_social_link" && get_option( 'wsl_settings_bouncer_linking_accounts_enabled' ) != 1 ){
		wsl_render_notices_pages( _wsl__("Bouncer say this makes no sense.", PIXELPIN_PLUGIN_CATEGORY) );
	...
	if( get_option( 'wsl_settings_bouncer_authentication_enabled' ) == 2 ){
		wsl_render_notices_pages( _wsl__("WSL is disabled!", PIXELPIN_PLUGIN_CATEGORY) ); 
...
function wsl_process_login_auth()
{
	...
	if( ! ( isset( $_REQUEST["provider"] ) && isset( $_REQUEST["redirect_to_provider"] ) ) ){ 
		wsl_render_notices_pages( _wsl__("Bouncer says this makes no sense.", PIXELPIN_PLUGIN_CATEGORY) ); 
	...
	try{
		...
		if( get_option( 'wsl_settings_bouncer_linking_accounts_enabled' ) != 1 && isset( $_REQUEST["link"] ) ){
			wp_die( _wsl__("Bouncer say you are doin it wrong.", PIXELPIN_PLUGIN_CATEGORY) );
		}
		...
		if( ! isset( $_REQUEST["link"] ) && is_user_logged_in() ){
			...
			wp_die( sprintf( _wsl__("You are already logged in as <b>%</b>.", PIXELPIN_PLUGIN_CATEGORY), $current_user->display_name ) );
		}

		# Hybrid_Auth already used?
		if ( class_exists('Hybrid_Auth', false) ) {
			return wsl_render_notices_pages( _wsl__("Error: Another plugin seems to be using HybridAuth Library and made PixelPin Social Login unusable. We recommand to find this plugin and to kill it with fire!", PIXELPIN_PLUGIN_CATEGORY) ); 
		}
		...
		if( ! get_option( 'wsl_settings_' . $provider . '_enabled' ) ){
			throw new Exception( _wsl__( 'Unknown or disabled provider', PIXELPIN_PLUGIN_CATEGORY) );
		}
		...
		if( get_option( 'wsl_settings_use_popup' ) == 1 || ! get_option( 'wsl_settings_use_popup' ) ){
			?>
				function init() {
                    if ( window.opener )
                    {
                        // Close popup if we are still in the popup
...
function wsl_process_login_render_error_page( $e, $config, $hybridauth, $adapter, $profile )
{
	...
	$message = _wsl__("Unspecified error!", PIXELPIN_PLUGIN_CATEGORY); 
	$hint    = ""; 

	switch( $e->getCode() ){
		case 0 : $message = _wsl__("Unspecified error.", PIXELPIN_PLUGIN_CATEGORY); break;
		case 1 : $message = _wsl__("Hybriauth configuration error.", PIXELPIN_PLUGIN_CATEGORY); break;
		case 2 : $message = _wsl__("Provider not properly configured.", PIXELPIN_PLUGIN_CATEGORY); break;
		case 3 : $message = _wsl__("Unknown or disabled provider.", PIXELPIN_PLUGIN_CATEGORY); break;
		case 4 : $message = _wsl__("Missing provider application credentials.", PIXELPIN_PLUGIN_CATEGORY); 
				 $hint    = sprintf( _wsl__("<b>What does this error mean ?</b><br />Most likely, you didn't setup the correct application credentials for this provider. These credentials are required in order for <b>%s</b> users to access your website and for PixelPin Social Login to work.", PIXELPIN_PLUGIN_CATEGORY), $provider ) . _wsl__('<br />Instructions for use can be found in the <a href="http://hybridauth.sourceforge.net/wsl/configure.html" target="_blank">User Manual</a>.', PIXELPIN_PLUGIN_CATEGORY); 
				 break;
		case 5 : $message = _wsl__("Authentification failed. The user has canceled the authentication or the provider refused the connection.", PIXELPIN_PLUGIN_CATEGORY); break; 
		case 6 : $message = _wsl__("User profile request failed. Most likely the user is not connected to the provider and he should to authenticate again.", PIXELPIN_PLUGIN_CATEGORY); 
				 if( is_object( $adapter ) ) $adapter->logout();
				 break;
		case 7 : $message = _wsl__("User not connected to the provider.", PIXELPIN_PLUGIN_CATEGORY); 
				 if( is_object( $adapter ) ) $adapter->logout();
				 break;
		case 8 : $message = _wsl__("Provider does not support this feature.", PIXELPIN_PLUGIN_CATEGORY); break;
...
<table width="100%" border="0">
	...
	<tr>
	<td align="center"><br /><h4><?php _wsl_e("Something bad happen!", PIXELPIN_PLUGIN_CATEGORY) ?></h4><br /></td> 
	</tr>
	<tr>
	<td align="center">
    ...
    <td align="center"> 
		<div style="padding: 5px;margin: 5px;background: none repeat scroll 0 0 #F5F5F5;border-radius:3px;">
			<div id="bug_report">
				...
				<small>
					<?php _wsl_e("Note: This message can be disabled from the plugin settings by setting <b>Development mode</b> to <b>Disabled</b>", PIXELPIN_PLUGIN_CATEGORY) ?>.
				</small>
			</div>
		</div>
	</td> 
...
<meta name="robots" content="NOINDEX, NOFOLLOW">
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<title><?php _wsl_e("Redirecting...", PIXELPIN_PLUGIN_CATEGORY) ?></title>
<head> 
<script>
function init(){
...
<table width="100%" border="0">
  <tr>
    <td align="center" height="40px"><br /><br /><?php echo sprintf( _wsl__( "Contacting <b>%s</b>, please wait...", PIXELPIN_PLUGIN_CATEGORY), ucfirst( $provider ) )  ?></td> 
  </tr> 
  <tr>
...
function wsl_process_login_reauth()
{
	...
	if( ! is_integer( $user_id ) ){
		return wsl_render_notices_pages( _wsl__("Invalid user_id returned by create_wp_user.", PIXELPIN_PLUGIN_CATEGORY) );
	}
...
function wsl_process_login_hybridauth_authenticate( $provider, $redirect_to )
{
	try{
			...
			if( get_option( 'wsl_settings_bouncer_linking_accounts_enabled' ) == 1 ){ 
				...
				if( $linked_account ){
					if( count( $linked_account ) > 1 ){
						return wsl_render_notices_pages( _wsl__("This $provider is linked to many accounts!", PIXELPIN_PLUGIN_CATEGORY) );
					}

					$user_id = $linked_account[0]->user_id;

					if( ! $user_id ){
						return wsl_render_notices_pages( _wsl__("Something wrong!", PIXELPIN_PLUGIN_CATEGORY) );
					}
		...
		# {{{ module Bouncer
			...
			if( ! $hybridauth_user_id ){
				...
				if( get_option( 'wsl_settings_bouncer_registration_enabled' ) == 2 ){
					return wsl_render_notices_pages( _wsl__("registration is now closed!", PIXELPIN_PLUGIN_CATEGORY) ); 
				}
		...
		return wsl_render_notices_pages( sprintf( _wsl__("Unspecified error. #%d", PIXELPIN_PLUGIN_CATEGORY), $e->getCode() ) ); 
	}
...
function wsl_process_login_create_wp_user( $provider, $hybridauth_user_profile, $request_user_login, $request_user_email )
{
	...
	// do not continue without user_id or we'll edit god knows what
		...
		if( is_wp_error( $user_id ) ){
			return wsl_render_notices_pages( _wsl__("An error occurred while creating a new user!" . $user_id->get_error_message(), PIXELPIN_PLUGIN_CATEGORY) );
		}

		return wsl_render_notices_pages( _wsl__("An error occurred while creating a new user!", PIXELPIN_PLUGIN_CATEGORY) );
	}
...
function wsl_process_login_authenticate_wp_user( $user_id, $provider, $redirect_to, $adapter, $hybridauth_user_profile )
{
	...
	if( !is_integer( $user_id ) ){
		return wsl_render_notices_pages( _wsl__("Invalid user_id", PIXELPIN_PLUGIN_CATEGORY) );
	}
