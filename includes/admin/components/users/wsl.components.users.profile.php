...
function wsl_component_users_profile( $user_id )
{
	...
	if( ! $linked_accounts ){
		_wsl_e("USER DOES NOT EXIST!", PIXELPIN_PLUGIN_CATEGORY);
	...
	<p>
		...
		<a class="button button-secondary" href="options-general.php?page=pixelpin-social-login&wslp=contacts&uid=<?php echo $user_id ?>">Show User Contacts List</a>
	</p>
	<hr />
	<?php
	foreach( $linked_accounts AS $link ){
		?>
			<h3><?php _wsl_e("User Profile", PIXELPIN_PLUGIN_CATEGORY); ?> <small><?php echo sprintf( _wsl__( "as provided by %s", PIXELPIN_PLUGIN_CATEGORY), $link->provider ); ?> </small></h3> 
