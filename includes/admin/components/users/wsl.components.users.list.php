...
<div style="margin-top:20px;">
	<table cellspacing="0" class="wp-list-table widefat fixed users">
		<thead>
			<tr> 
				<th width="100"><span><?php _wsl_e("Providers", PIXELPIN_PLUGIN_CATEGORY) ?></span></th>  
				<th><span><?php _wsl_e("Username", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th><span><?php _wsl_e("Full Name", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th><span><?php _wsl_e("E-mail", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th><span><?php _wsl_e("Profile URL", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th width="60"><span><?php _wsl_e("Contacts", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th width="140"><span><?php _wsl_e("Actions", PIXELPIN_PLUGIN_CATEGORY) ?></span></th>
			</tr>
		</thead> 
		<tfoot>
			<tr> 
				...
				<th><span><?php _wsl_e("E-mail", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th><span><?php _wsl_e("Profile URL", PIXELPIN_PLUGIN_CATEGORY) ?></span></th> 
				<th><span><?php _wsl_e("Contacts", PIXELPIN_PLUGIN_CATEGORY) ?></span></th>
				<th><span><?php _wsl_e("Actions", PIXELPIN_PLUGIN_CATEGORY) ?></span></th>
			</tr>
		</tfoot> 
		<tbody data-wp-lists="list:user" id="the-list">
			<?php  
				...
				if( ! $rs1 ){
					?>
						<tr class="no-items"><td colspan="6" class="colspanchange"><?php _wsl_e("No users found", PIXELPIN_PLUGIN_CATEGORY) ?>.</td></tr>
					<?php
				}
				else{
					$i = 0;
			...
			?>
					<tr class="<?php if( ++$i % 2 ) echo "alternate" ?> tr-contacts"> 
						...
						<td>
							<a class="button button-secondary" href="options-general.php?page=pixelpin-social-login&wslp=users&uid=<?php echo $user_id ?>">Profile</a>
							<a class="button button-secondary" href="options-general.php?page=pixelpin-social-login&wslp=contacts&uid=<?php echo $user_id ?>">Contacts</a>
						</td> 
					</tr> 
			<?php 
					}
