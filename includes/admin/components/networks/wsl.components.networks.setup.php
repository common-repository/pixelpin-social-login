...
<script>
	function toggleproviderkeys(idp){
		if(typeof jQuery=="undefined"){
			alert( "Error: PixelPin Social Login require jQuery to be installed on your wordpress in order to work!" );
		...
		<div class="stuffbox" id="namediv">
			...
			<div class="inside">
				<table class="form-table editcomment">
					<tbody>
						<tr>
							<td style="width:110px"><?php _wsl_e("Enabled", PIXELPIN_PLUGIN_CATEGORY) ?>:</td>
							<td>
								<select 
						...
						<?php if ( $provider_new_app_link ){ ?>
							<?php if ( $require_client_id ){ // key or id ? ?>
								<tr valign="top" <?php if( ! get_option( 'wsl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="wsl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _wsl_e("Application ID", PIXELPIN_PLUGIN_CATEGORY) ?>:</td>
									...
									<td><a href="javascript:void(0)" onClick="toggleproviderhelp('<?php echo $provider_id; ?>')"><?php _wsl_e("Where do I get this info?", PIXELPIN_PLUGIN_CATEGORY) ?></a></td>
								</tr> 
							<?php } else { ?>
								<tr valign="top" <?php if( ! get_option( 'wsl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="wsl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _wsl_e("Application Key", PIXELPIN_PLUGIN_CATEGORY) ?>:</td>
							...
							<?php }; ?>	 
								<tr valign="top" <?php if( ! get_option( 'wsl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="wsl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _wsl_e("Application Secret", PIXELPIN_PLUGIN_CATEGORY) ?>:</td>
				...
				<p style="margin-left:12px;margin-bottom:0px;"> 
					<b  style="color:#CB4B16;"><?php _wsl_e("Note", PIXELPIN_PLUGIN_CATEGORY) ?>:</b> 
					
					<?php echo sprintf( _wsl__("<b>%s</b> do not provide their user's email address and by default a random email will then be generated for them instead", PIXELPIN_PLUGIN_CATEGORY), $provider_name ) ?>. 
					
					<?php _wsl_e('To change this behaviour and to force new registered users to provide their emails before they get in, goto <b><a href="options-general.php?page=pixelpin-social-login&wslp=bouncer">Bouncer</a></b> and enable <b>Profile Completion</b>', PIXELPIN_PLUGIN_CATEGORY) ?>.
				</p>
				<?php endif; ?> 
				<br />
				<div
				...
				> 
					<hr class="wsl" />
					<?php if ( $provider_new_app_link  ) : ?> 
						<?php _wsl_e('<span style="color:#CB4B16;">Application</span> id and secret (also sometimes referred as <span style="color:#CB4B16;">Customer</span> key and secret or <span style="color:#CB4B16;">Client</span> id and secret) are what we call an application credentials', PIXELPIN_PLUGIN_CATEGORY) ?>. 
						
						<?php echo sprintf( _wsl__( 'This application will link your website <code>%s</code> to <code>%s API</code> and these credentials are needed in order for <b>%s</b> users to access your website', PIXELPIN_PLUGIN_CATEGORY), $_SERVER["SERVER_NAME"], $provider_name, $provider_name ) ?>. 
						<br />
						
						<?php _wsl_e("These credentials may also differ in format, name and content depending on the social network.", PIXELPIN_PLUGIN_CATEGORY) ?> 
						<br />
						<br />
						
						<?php echo sprintf( _wsl__('To enable authentication with this provider and to register a new <b>%s API Application</b>, carefully follow the steps', PIXELPIN_PLUGIN_CATEGORY), $provider_name ) ?>
						:<br />
					<?php else: ?>  
							<p><?php echo sprintf( _wsl__('<b>Done.</b> Nothing more required for <b>%s</b>', PIXELPIN_PLUGIN_CATEGORY), $provider_name) ?>.</p> 
					<?php endif; ?>  
