...
function wsl_check_compatibilities()
{
	...
	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_notice' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_notice', _wsl__("Almost there, we just need to check a couple of things", PIXELPIN_PLUGIN_CATEGORY) );
	}

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_submit_button' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_submit_button', _wsl__("Continue", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_email' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_email', _wsl__("E-mail", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_username' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_username', _wsl__("Username", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_email_invalid' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_email_invalid', _wsl__("E-mail is not valid!", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_username_invalid' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_username_invalid', _wsl__("Username is not valid!", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_email_exists' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_email_exists', _wsl__("That E-mail is already registered!", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_username_exists' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_username_exists', _wsl__("That Username is already registered!", PIXELPIN_PLUGIN_CATEGORY) );
	} 

	if( ! get_option( 'wsl_settings_bouncer_profile_completion_text_connected_with' ) ){
		update_option( 'wsl_settings_bouncer_profile_completion_text_connected_with', _wsl__("You are now connected via", PIXELPIN_PLUGIN_CATEGORY) );
	} 
	...
	if( ! get_option( 'wsl_settings_bouncer_new_users_restrict_domain_text_bounce' ) ){
		update_option( 'wsl_settings_bouncer_new_users_restrict_domain_text_bounce', _wsl__("Bouncer says no.", PIXELPIN_PLUGIN_CATEGORY) );
	}
	...
	if( ! get_option( 'wsl_settings_bouncer_new_users_restrict_email_text_bounce' ) ){
		update_option( 'wsl_settings_bouncer_new_users_restrict_email_text_bounce', _wsl__("Bouncer say he refuses.", PIXELPIN_PLUGIN_CATEGORY) );
	}
	...
	if( ! get_option( 'wsl_settings_bouncer_new_users_restrict_profile_text_bounce' ) ){
		update_option( 'wsl_settings_bouncer_new_users_restrict_profile_text_bounce', _wsl__("Bouncer say only Mundo can go where he pleases!", PIXELPIN_PLUGIN_CATEGORY) );
	}
